...
router.get("/s/:url", async (request) => {
  ...
  if (!value) {
    ...
    console.log(`publishing msg: ${JSON.stringify(message)}`)

    const p = kafka.producer()
    
    await p.produce("visits-log", message).then(val => {
      console.log(`published to kafka topic "visits-log": ${JSON.stringify(val)}`)
    }).catch(err => {
      console.error(`ERR publishing to kafka topic "visits-log": ${err}`)
    })
